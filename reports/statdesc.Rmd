...
df_reg_ohe <- dummy_cols(df_reg, select_columns =  c("feduc","meduc","mrace6","frace6","fagerec11","dob_yy"), remove_first_dummy = TRUE)

# Removing initial columns 
df_reg_ohe$feduc <- NULL
df_reg_ohe$Meduc <- NULL
df_reg_ohe$meduc <- NULL
df_reg_ohe$frace6 <- NULL
df_reg_ohe$mrace6 <- NULL
df_reg_ohe$fagerec11 <- NULL
df_reg_ohe$dob_yy <- NULL

#Linear regression of ldbwt on ohe variables 

res_ohe <- lm(ldbwt ~ .,
          data = df_reg_ohe)
summary(res_ohe)

#df %>% select(dob_yy) %>% distinct()

```

```{r, results='asis'}
stargazer::stargazer(res_ohe, type = "html")
```
